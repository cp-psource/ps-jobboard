	...
	function menu() {
		...
		<li <?php echo je()->get( 'tab' ) == 'expert_demo_data' ? 'class="active"' : null ?>>
			<a href="<?php echo admin_url( 'edit.php?post_type=jbp_job&page=jobs-plus-menu&tab=expert_demo_data' ) ?>">
				<i class="dashicons dashicons-star-filled"></i> <?php _e( 'Experten Demo-Daten', 'psjb' ) ?>
			</a></li>
	<?php
	}
		...
		<form method="post" id="expert-demo-data">
			<fieldset>
				<div class="page-header" style="margin-top: 0">
					<h4><?php _e( 'Erstelle Dummy-Daten für Experten', 'psjb' ) ?></h4>
				</div>

				<div class="form-group">
					<label class="col-md-3 label-control"><?php _e( 'Dummy Experten Menge', 'psjb' ) ?></label>

					<div class="col-md-9">
						<input type="text" value="10" name="dummy_expert_qty" class="form-control">

						<p class="help-block"><?php _e( 'Anzahl der zu erstellenden Dummy-Experten', 'psjb' ) ?></p>
					</div>
					<div class="clearfix"></div>
				</div>
				<div class="form-group">
					<label class="col-md-3 label-control"><?php _e( 'Soziales Profil', 'psjb' ) ?></label>
					...
					<div class="col-md-9">
						...
						<p class="help-block"><?php _e( 'Soziales Profil für Experten. Wenn leer, werden 3-5 Profile zufällig hinzugefügt', 'psjb' ) ?></p>
					</div>
					<div class="clearfix"></div>
				</div>
				<div class="form-group">
					<label class="col-md-3 label-control"><?php _e( 'Fähigkeiten', 'psjb' ) ?></label>

					<div class="col-md-9">
						<input type="text" value="Html,CSS,Javascript,jQuery,Bootstrap,PHP,NodeJS,MySQL"
						       name="dummy_skills" class="form-control">

						<p class="help-block"><?php _e( 'Experten Fähigkeiten. Die Punktzahl ist zufällig von 0-100.', 'psjb' ) ?></p>
					</div>
					<div class="clearfix"></div>
				</div>
				<div class="form-group">
					<label class="col-md-3 label-control"><?php _e( 'Beispieldateien?', 'psjb' ) ?></label>

					<div class="col-md-9">
						<p class="help-block">
							<input type="checkbox" checked="checked" name="have_sample">
							<?php _e( 'Wenn Du möchtest, dass die Demo-Experten Beispieldateien haben, aktiviere dieses Kontrollkästchen.', 'psjb' ) ?>
						</p>
					</div>
					<div class="clearfix"></div>
				</div>
				<div class="demo_status hide">
					<p><?php _e( 'Demodaten erstellen, bitte warten....', 'psjb') ?></p>
				...
				<div class="row">
					<div class="col-md-9 col-md-offset-3">
						<button type="submit" class="btn btn-primary"><?php _e( "Hinzufügen", 'psjb' ) ?></button>
					</div>
				</div>
			</fieldset>
		</form>
				...
				$('#expert-demo-data').submit(function () {
					...
					$.ajax({
						...
						beforeSend: function () {
							...
							$('.demo_status').find('p').text('Demodaten erstellen, bitte warten....', 'psjb');
							//triger load status
									...
									success: function (data) {
										if (data.length) {
											if (data == 100) {
												...
												$('.demo_status').find('p').text('Erledigt!', 'psjb');
