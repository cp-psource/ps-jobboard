...
/**
 * Name: Experte Geo Location
 * Description: Lasse Experten ihren Standort definieren.
 * Author: WMS N@W
 */
class JE_Custom_Location extends IG_Request
{
    ...
    function save_setting()
    {
        ...
        if (je()->post('api', null) !== null) {
            ...
            if ($geo['status'] == true) {
                ...
                $this->set_flash('key', __("Der API-SCHLÜSSEL wurde erfolgreich gespeichert!"));
    ...
    function menu()
    {
        ...
        <li <?php echo je()->get('tab') == 'custom_location' ? 'class="active"' : null ?>>
            <a href="<?php echo admin_url('edit.php?post_type=jbp_job&page=jobs-plus-menu&tab=custom_location') ?>">
                <i class="fa fa-crosshairs"></i> <?php _e('Benutzerdefinierter Expertenstandort', 'psjb') ?>
            </a></li>
    <?php
    }
    ...
    function content()
    {
        ...
        <form method="post">
            <div class="page-header" style="margin-top: 0">
                <h3><?php _e('Benutzerdefinierter Expertenstandort', 'psjb') ?></h3>
            </div>
    ...
    function location_span($html, $model)
    {
        ...
        $location = !empty($model->location) ? $model->location : __("Dein Standort", 'psjb');
        $html .= $location;
        $html .= '</span>';
