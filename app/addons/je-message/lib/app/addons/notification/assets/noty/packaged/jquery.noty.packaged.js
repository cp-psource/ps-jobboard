 ...
 */
    ...
    var NotyObject = {
        ...
        _build: function() {
            ...
            // Generating noty bar
            var $bar = jQuery('<div class="noty_bar noty_type_' + this.options.type + '"></div>').attr('id', this.options.id);
            $bar.append(this.options.template).find('.noty_text').html(this.options.text);

            this.$bar = (this.options.layout.parent.object !== null) ? jQuery(this.options.layout.parent.object).css(this.options.layout.parent.css).append($bar) : $bar;
            ...
            if(this.options.buttons) {
                ...
                var $buttons = jQuery('<div/>').addClass('noty_buttons');
                ...
                jQuery.each(this.options.buttons, function(i, button) {
                    var $button = jQuery('<button/>').addClass((button.addClass) ? button.addClass : 'gray').html(button.text).attr('id', button.id ? button.id : 'button-' + i)
        ...
        show: function() {
            ...
            (self.options.custom) ? self.options.custom.find(self.options.layout.container.selector).append(self.$bar) : jQuery(self.options.layout.container.selector).append(self.$bar);

            if(self.options.theme && self.options.theme.style)
                self.options.theme.style.apply(self);

            (jQuery.type(self.options.layout.css) === 'function') ? this.options.layout.css.apply(self.$bar) : self.$bar.css(this.options.layout.css || {});
    ...
    jQuery.notyRenderer.createModalFor = function(notification) {
        if(jQuery('.noty_modal').length == 0) {
            ...
            if(jQuery.inArray('backdrop', notification.options.closeWith) > -1)
...
jQuery.noty.themes.bootstrapTheme = {
    name: 'bootstrapTheme',
    modal: {
        css: {
            position: 'fixed',
            ...
            if(this.options.dismissQueue) {
                ...
                switch(this.options.layout.name) {
                    case 'top':
                        ...
                        jQuery(selector).last().css({borderRadius: '0px 0px 5px 5px'});
                        break;
                    case 'topCenter':
                    case 'topLeft':
                    case 'topRight':
                    ...
                    case 'inline':
                        ...
                        jQuery(selector).last().css({'border-bottom-left-radius': '5px', 'border-bottom-right-radius': '5px'});
                        break;
                    case 'bottom':
                        jQuery(selector).css({borderRadius: '0px 0px 0px 0px'});
                        jQuery(selector).first().css({borderRadius: '5px 5px 0px 0px'});
                        break;
                    default:
                        break;
                }
