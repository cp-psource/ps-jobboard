...
class JE_Job_Search_Widget_Controller extends WP_Widget
{
    ...
    function widget($args, $instance)
    {
        ...
        $title = apply_filters('widget_title', empty($instance['title']) ? __('Jobs suchen', 'psjb') : $instance['title'], $instance, $this->id_base);
        echo $before_widget;
        if ($title) {
                        ...
                        <div class="col-md-12">
                            <div class="well well-sm ">
                                <form class="search-form" id="<?php echo $form_id ?>" method="GET"
                                    ...
                                    <button type="submit"
                                            class="btn btn-primary btn-sm pull-right"><?php _e('Suchen', 'psjb') ?></button>
                                    <div class="clearfix"></div>
                                </form>
                            </div>
                        </div>
        ...
        <p>
            ...
            <select name="<?php echo esc_attr($this->get_field_name('view')); ?>">
                ...
                <option <?php echo selected('loggedout', $view) ?>
                    value="loggedout"><?php _e('Nicht angemeldet', 'psjb') ?></option>
            </select>
        </p>
    <?php
    }
