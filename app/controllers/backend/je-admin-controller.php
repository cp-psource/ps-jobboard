	...
	function menu_order( $menu_order ) {
		...
		if ( isset( $submenu['edit.php?post_type=jbp_job'] ) ) {
			...
			foreach ( $nav as $key => &$item ) {
				if ( is_array( $item ) ) {
					if ( $item[0] == __( 'Übersichtsseiten', 'psjb' ) ) {
						$getting_start = array(
							$key => $item
						);
						unset( $nav[ $key ] );
					} elseif ( $item[0] == $job->labels->name ) {
						$item[0] = sprintf( __( 'Verwalte %s', 'psjb' ), $job->labels->name );
					} elseif ( $item[0] == __( 'Neuen hinzufügen', 'psjb' ) ) {
						//unset($nav[$key]);
						//finding the New Job and fix it to this posiston
						foreach ( $nav as $k => $v ) {
							if ( is_array( $v ) && $v[0] == sprintf( __( 'Neuer %s', 'psjb' ), $job->labels->singular_name ) ) {
								$nav[ $key ] = $v;
								unset( $nav[ $k ] );
								break;
							}
			...
			foreach ( $nav as $key => &$item ) {
				if ( is_array( $item ) ) {
					if ( $item[0] == $pro->labels->name ) {
						$item[0] = sprintf( __( 'Verwalte %s', 'psjb' ), $pro->labels->name );
					} elseif ( $item[0] == __( 'Neuen hinzufügen', 'psjb' ) ) {
						//unset( $nav[ $key ] );
						$item[0] = sprintf( __( "Neuer %s", 'psjb' ), $pro->labels->singular_name );
					}
				}
			}
