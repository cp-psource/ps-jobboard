...
class JE_Job_Admin_Controller extends IG_Request {
	...
	function metaboxes() {
		add_meta_box( 'job-meta', __( "Job Meta", 'psjb' ), array(
			&$this,
			'show_job_meta'
		), 'jbp_job', 'advanced', 'high' );

		add_meta_box( 'job-attachments', __( "Füge Beispiele für zusätzliche Informationen bei", 'psjb' ), array(
			&$this,
			'show_job_attachment'
		), 'jbp_job', 'advanced', 'high' );
	}
	...
	public function show_job_attachment() {
		...
		ig_uploader()->show_upload_control( $model, 'portfolios', true, array(
			'title' => __( "Füge Beispiele für zusätzliche Informationen bei", 'psjb' )
		) );
		echo '</div>';
	}
	...
	function process_request() {
		...
		if ( $model->validate() ) {
			$model->save();
			$this->set_flash( 'job_saved', __( "Job gespeichert!" ) );
	...
	function edit_job() {
		...
		if ( ! is_object( $model ) ) {
			echo __( "Job nicht gefunden!", 'psjb' );
		}
