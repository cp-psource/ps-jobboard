...
class JE_Expert_Admin_Controller extends IG_Request {
	...
	function metaboxes() {
		add_meta_box( 'Addition Information', __( "Job Meta", 'psjb' ), array(
			&$this,
			'show_addition_information'
		), 'jbp_pro', 'advanced', 'high' );
		add_meta_box( 'social_skill', __( "Social Profile & Skill", 'psjb' ), array(
			&$this,
			'social_skill'
		), 'jbp_pro', 'advanced', 'high' );

		add_meta_box( 'expert-attachments', __( "Füge Beispiele für zusätzliche Informationen bei", 'psjb' ), array(
			&$this,
			'show_job_attachment'
		), 'jbp_pro', 'advanced', 'high' );
	}
	...
	public function show_job_attachment() {
		...
		ig_uploader()->show_upload_control( $model, 'portfolios', true,
			array( 'title' => __( "Füge Beispiele für zusätzliche Informationen bei", 'psjb' ) ) );
		echo '</div>';
	}
	...
	public function show_addition_information() {
		...
		<table class="widefat">
			<tr>
				<th valign="top">
					<?php $form->label( "contact_email", array( "text" => __( "Kontakt Email", 'psjb' ) ) ) ?>
				</th>
				<td>
	...
	function table_columns( $columns ) {
		$columns['author'] = __( "Benutzer", 'psjb' );
		$columns['title']  = __( "Name", 'psjb' );
	...
	function process_request() {
		...
		if ( $model->validate() ) {
			$model->save();
			$this->set_flash( 'expert_saved', __( "Expertenprofil gespeichert!" ) );
	...
	function edit_expert() {
		...
		if ( ! is_object( $model ) ) {
			echo __( "Experte nicht gefunden!", 'psjb' );
		}
