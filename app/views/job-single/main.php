					...
					<?php if ( strtolower( $model->get_due_day() ) != 'expired' && je()->settings()->job_contact_form == 0 ): ?>
						<?php if ( JobsExperts_Helper::is_user_pro( get_current_user_id() ) ): ?>
							...
							<a class="btn btn-info btn-sm jbp_contact_job"
							   ...
							   ), apply_filters( 'jbp_job_contact_link', get_permalink( je()->pages->page( JE_Page_Factory::JOB_CONTACT ) ), get_the_ID() ) ) ) ?>"><?php _e( 'Kontakt', 'psjb' ) ?></a>
				...
				<div class="col-md-12">
					...
					if ( ! empty( $skills ) ): ?>
						<div class="job_skills">
							<?php
							echo get_the_term_list( $model->id, 'jbp_skills_tag', __( '<h4>Du musst diese Fähigkeiten haben:', 'psjb' ) . '</h4><ul><li>', '</li><li>', '</li></ul>' )
							?>
						</div>
					<?php endif; ?>
				</div>
					...
					?>
						...
						<form class="frm-delete" method="post" style="display: inline-block">
							...
							<button name="delete_job" class="btn btn-danger"
							        type="submit"><?php _e( 'Papierkorb', 'psjb' ) ?></button>
						</form>
