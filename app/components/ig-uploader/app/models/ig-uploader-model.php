			...
			if ( $this->mode == self::MODE_EXTEND ) {
				if ( ( $this->exist && empty( $this->file ) ) || ! $this->exist ) {
					...
					if ( empty( $this->url ) && empty( $this->file ) ) {
						$this->set_error( 'file', __( "URL oder Datei erforderlich", ig_uploader()->domain ) );
						$this->set_error( 'url', __( "URL oder Datei erforderlich", ig_uploader()->domain ) );
					}
				}
			}
