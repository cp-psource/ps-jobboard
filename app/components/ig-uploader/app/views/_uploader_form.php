<div class="ig-container">
    ...
    <div style="margin-bottom: 0" class="form-group <?php echo $model->has_error("file") ? "has-error" : null ?>">
        <button type="button"
                class="btn btn-default upload_image_button btn-xs"><?php _e("Datei wählen", ig_uploader()->domain) ?></button>
    ...
    <div style="margin-bottom: 0" class="form-group <?php echo $model->has_error("content") ? "has-error" : null ?>">
        <?php $form->label("content", array("text" => __("Beschreibung", ig_uploader()->domain), "attributes" => array("class" => "control-label hidden-xs hidden-sm"))) ?>
        <?php $form->text_area("content", array("attributes" => array("class" => "form-control input-sm", "style" => "height:80px", "placeholder" => __("Beschreibung", ig_uploader()->domain)))) ?>
