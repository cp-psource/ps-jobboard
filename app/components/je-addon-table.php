...
class JE_AddOn_Table extends WP_List_Table
{
    ...
    function get_columns()
    {
        return $columns = array(
            'col_name' => __('Name', 'psjb'),
            'col_description' => __('Beschreibung', 'psjb'),
        );
    }
        ...
        if (in_array($item['col_id'], $components)) {
            ...
            if (!empty($item['required'])) {
                if (is_plugin_active($item['required'])) {
                    $html .= '<br><a class="mm-plugin" data-type="active" data-id="' . esc_attr($item['col_id']) . '"  href="#">' . __('Aktiviere <i class="fa fa-circle-o-notch fa-spin loader-ani hide"></i>', 'psjb') . '</a>';
                } else {
    ...
    {
        return $item['col_description'] . '<br/>' . __('Erstellt von: ', 'psjb') . '<strong>' . $item['col_author'] . '</strong>';
    }
